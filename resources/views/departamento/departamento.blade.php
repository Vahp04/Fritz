...
<head>
    ...
    <title>Fritz C.A | Gestión de Departamentos</title>
    ...
    <style>
       ...
       .avatar-circle {
        ...
        .department-icon {
            ...
            height: 50px;
            border-radius: 10px;
            background: linear-gradient(135deg, #7cea66ff 0%, #b4c04cff 100%);
            display: flex;
            align-items: center;
            justify-content: center;
            color: white;
...
<body>
    <div class="d-flex">
        ...
        <div class="sidebar text-white p-3 position-relative">
            ...
            <ul class="nav flex-column">
                ...
                <li class="nav-item">
                    <a class="nav-link text-white sidebar-link active" href="{{ route('departamentos.index') }}">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb mb-0">
                                ...
                                <li class="breadcrumb-item active text-white" aria-current="page">Gestión de Departamentos</li>
                            </ol>
                        </nav>
                    </div>
            ...
            <main class="p-4">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        ...
                        <p class="text-muted mb-0">Administra los departamentos del sistema</p>
                    </div>
                    <div>
                        <button class="btn btn-fritz" data-bs-toggle="modal" data-bs-target="#createDepartmentModal">
                ...
                <div class="card border-0 shadow mb-4">
                    <div class="card-header bg-dark text-white py-3">
                        <div class="d-flex justify-content-between align-items-center">
                            <h5 class="mb-0">
                                <i class="bi bi-list-ul me-2 text-danger"></i> Lista de Departamentos
                            </h5>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="fw-bold">{{ $departamento->id }}</td>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="department-icon me-3">
                                                    {{ strtoupper(substr($departamento->nombre, 0, 1)) }}
                                                </div>
                                                <div>
                                                    <div class="fw-bold">{{ $departamento->nombre }}</div>
                                                    <small class="text-muted">ID: {{ $departamento->id }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="badge rounded-pill bg-primary">
                                                ...
                                                {{ $departamento->usuarios_count ?? 0 }} Usuarios
                                            </span>
                                        </td>
                                        <td>
                                        ...
                                        <td class="table-actions text-center">
                                            <button class="btn btn-sm btn-outline-info action-btn me-1" 
                                                    ...
                                                    onclick="viewDepartmentData({{ $departamento }})"
                                                    title="Ver">
                                                <i class="bi bi-eye"></i>
                                            </button>
                                            <button class="btn btn-sm btn-outline-warning action-btn me-1" 
                                                    ...
                                                    onclick="loadDepartmentData({{ $departamento }})"
                                                    title="Editar">
                                                <i class="bi bi-pencil"></i>
                                            </button>
                                            <button class="btn btn-sm btn-outline-danger action-btn"
                                                    onclick="confirmDelete({{ $departamento->id }}, '{{ $departamento->nombre }}')"
                                                    title="Eliminar">
            ...
            <div class="modal-content">
                <div class="modal-header bg-dark text-white">
                    <h5 class="modal-title">
                        <i class="bi bi-person-badge me-2"></i> Nuevo Departamento
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <form action="{{ route('departamentos.store') }}" method="POST" id="createDepartmentForm">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label">Nombre del Departamento*</label>
                            <input type="text" class="form-control" name="nombre" required placeholder="Ingrese el nombre del departamento">
                            <small class="text-muted">Ej: Recursos Humanos, Tecnología, Ventas, etc.</small>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                        <button type="submit" class="btn btn-fritz">Guardar Departamento</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-info text-white">
                    <h5 class="modal-title">
                        <i class="bi bi-eye me-2"></i> Detalles del Departamento
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="text-center mb-4">
                        <div class="department-icon mx-auto mb-3" style="width: 80px; height: 80px; font-size: 2rem;" id="view_icon">
                    ...
                    <div class="row">
                        <div class="col-6">
                            <strong>ID:</strong>
                            <p id="view_department_id" class="mb-2"></p>
                        </div>
                        <div class="col-6">
    ...
    <script>
        ...
        function loadDepartmentData(department) {
            document.getElementById('editDepartmentForm').action = `/departamentos/${department.id}`;
            document.getElementById('edit_nombre').value = department.nombre;
        }

        // Función para cargar datos en el modal de visualización
        function viewDepartmentData(department) {
            document.getElementById('view_icon').textContent = department.nombre.charAt(0).toUpperCase();
            document.getElementById('view_nombre').textContent = department.nombre;
            document.getElementById('view_id').textContent = `ID: ${department.id}`;
            document.getElementById('view_department_id').textContent = department.id;
            document.getElementById('view_usuarios_count').innerHTML = department.usuarios_count ? 
                `<span class="badge bg-primary">${department.usuarios_count} Usuarios</span>` : 
                '<span class="badge bg-secondary">0 Usuarios</span>';
            document.getElementById('view_created').textContent = new Date(department.created_at).toLocaleDateString('es-ES');
            document.getElementById('view_updated').textContent = new Date(department.updated_at).toLocaleDateString('es-ES');
            
           
            $('#viewDepartmentModall').modal('show');
        ...
        function confirmDelete(departmentId, departmentName) {
            Swal.fire({
                title: '¿Estás seguro?',
                text: `Vas a eliminar el departamento "${departmentName}". Esta acción no se puede deshacer.`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = `/departamentos/${departmentId}`;
        ...
        $(document).ready(function() {
            $('#applyFilters').click(function() {
                ...
                console.log('Aplicar filtros:', { nombre: filterName });
