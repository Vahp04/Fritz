...
<head>
    <meta charset="UTF-8">
    <title>Equipos Asignados - {{ $usuario->nombre }}</title>
    <style>
        body {
        ...
        /* ESTILOS MEJORADOS PARA FIRMAS - SOLUCIÓN CON TABLA */
        .firmas-container {
            margin-top: 60px;
            width: 100%;
...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $asignacion->id }}</td>
                <td>
                    <strong>{{ $asignacion->stock_equipo->marca ?? 'N/A' }} {{ $asignacion->stock_equipo->modelo ?? '' }}</strong>
                    <br><small>{{ $asignacion->stock_equipo->descripcion ?? '' }}</small>
                </td>
                <td>{{ $asignacion->stock_equipo->tipoEquipo->nombre ?? 'Sin tipo' }}</td>
                ...
                <td>{{ $asignacion->ip_equipo ?? '-' }}</td>
                <td class="text-center">
                    @if($asignacion->estado == 'activo')
                        <span class="badge badge-success">Activo</span>
                    @elseif($asignacion->estado == 'devuelto')
                        <span class="badge badge-info">Devuelto</span>
                    @elseif($asignacion->estado == 'obsoleto')
                        <span class="badge badge-warning">Obsoleto</span>
                    @else
                        <span class="badge badge-secondary">{{ $asignacion->estado }}</span>
                    @endif
                </td>
                <td>{{ $asignacion->usuario->name ?? 'Sistema' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div style="text-align: center; padding: 20px; background: #f8f9fa; border-radius: 5px;">
        <p style="margin: 0; color: #666; font-style: italic;">El usuario no tiene equipos asignados</p>
    </div>
    @endif

    <!-- SECCIÓN DE FIRMAS CON TABLA - MÁS COMPATIBLE -->
    <div class="firmas-container">
        <table class="tabla-firmas">
            <tr>
                ...
                <!-- Firma del Departamento de Tecnología -->
                <td>
                    <div class="firma">
                        <div class="espacio-firma"></div>
    ...
    <div class="footer">
        ...
        <p>FRITZ C.A - Sistema de Gestión de Equipos</p>
    </div>
</body>
</html>